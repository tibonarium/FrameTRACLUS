...
import TraClusterAlgorithm.ComputationWorker.Parameter;

public class ClusterGen {

    public ComputationWorker m_document;
    ...
    //private ArrayList<LineSegmentCluster> m_lineSegmentClusters;
    ...
    public class LineSegmentCluster {
        ...
        public ArrayList<CMDPoint> getClusterPointArray() {
            return this.clusterPointArray;
        }
    ...
    public ClusterGen(ComputationWorker document) {
...
//		m_projectionPoint = new CMDPoint();
    ...
    boolean constructLineSegmentCluster() {
        ...
        // New code
        /*
        ArrayList<LineSegmentCluster> m_lineSegmentClusters = new ArrayList<LineSegmentCluster>();

        LineSegmentCluster objLineSegmentCluster = new LineSegmentCluster();

        objLineSegmentCluster.avgDirectionVector = new CMDPoint();
        objLineSegmentCluster.lineSegmentClusterId = i;
        objLineSegmentCluster.nLineSegments = 0;
        objLineSegmentCluster.nClusterPoints = 0;
        objLineSegmentCluster.nTrajectories = 0;
        objLineSegmentCluster.enabled = false;
        */
        ...
        for (int i = 0; i < m_currComponentId; i++)
        {
            ...
            //m_lineSegmentClusters.add(objLineSegmentCluster);
    ...
    private void computeRepresentativeLines(LineSegmentCluster clusterEntry) {
        ...
        while(iter != (clusterEntry.candidatePointList.size()-1)
                && clusterEntry.candidatePointList.size()>0)	{
            insertionList.clear();
            deletionList.clear();
    ...
    public ArrayList<LineSegmentCluster> getRepresentativeTrajectory() {

        ArrayList<LineSegmentCluster> lineSegmentClusters = new ArrayList<LineSegmentCluster>();

        for(int i=0; i<m_lineSegmentClusters.length; i++) {
            lineSegmentClusters.add(m_lineSegmentClusters[i]);
        }

        return lineSegmentClusters;
    }
}
